...
d3 = function() {
    ...
    function makeScroller(scrollToPath) {
        return function() {
            ...
            scrollTo(window, document.getElementById(path).offsetTop, 1e3), window.history.pushState({
                path: path
            }, "bbbench " + path, "/#" + path);
        };
    }
    for (var scrollers = document.getElementsByClassName("scroller"), length = scrollers.length, i = 0; length > i; i++) scrollers[i].onclick = makeScroller(scrollers[i].dataset.scrollTo);
...
function(window, document) {
    ...
    function addProfile(profile, profileName) {
        ...
        var rb = document.getElementById("reload-button"), counters = document.getElementById("counters"), oldIndex = counters.children.length;
        ...
        }, counters.children[oldIndex].children[0].onclick = function() {
    ...
    function drawMe() {
        ...
        {
            var divvy = window.d3.select("#divvy"), n = starkLines.data.length, m = starkLines.data[0].length, stack = window.d3.layout.stack(), layers = stack(starkLines.data.map(function(dd) {
                ...
                bottom: 0,
                left: 0
            }, width = document.getElementById("graph1").offsetWidth - margin.left - margin.right, height = document.getElementById("graph1").offsetHeight - margin.top - margin.bottom, x = window.d3.scale.ordinal().domain(window.d3.range(m)).rangeRoundBands([ 0, width ], .05), y = window.d3.scale.linear().domain([ 0, yStackMax() ]).range([ height, 0 ]), color = window.d3.scale.linear().domain([ 0, n - 1 ]).range([ "#24B1E0", "#F80F40" ]), svg = (window.d3.svg.axis().scale(x).tickSize(0).tickPadding(6).orient("bottom"), 
    ...
    function runnable() {
        ...
        setTimeout(drawMe, 2e3), starkLines.runningInterval = setInterval(function() {
            for (i = 0; i < starkLines.profileList.length; i++) setTimeout(queryLiveReportingApi, 110, starkLines.profileList[i].id);
    ...
    var color = window.d3.scale.linear().domain([ 0, starkLines.data.length - 1 ]).range([ "#24B1E0", "#F80F40" ]);
    !function() {
