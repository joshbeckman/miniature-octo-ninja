...
function(window, document) {
    ...
    function handleWebproperties(results) {
        if (results.code) console.log(results), generalAlert({
            ...
            appendingList.push(dndCtaTemplate()), optionsDiv.innerHTML = appendingList.join(""), 
            readyChoiceLinks(queryProfiles), document.getElementById("reload-button").className = "";
    ...
    function addProfile(profile, profileName) {
        ...
        rb.innerHTML = "Add Profile", rb.onclick = function() {
            queryAccounts();
        }, optionsDiv.innerHTML = null, counters.innerHTML += profileTemplate({
            ...
            id: profile,
            name: profileName
        }), starkLines.running === !1 && runnable(), starkLines.profileList.length == starkLines.data.length && (rb.style.display = "none"), 
    ...
    function pusher(position, results) {
        ...
        }) : 0, el = document.getElementById(results.profileInfo.profileId.toString() + "-counter");
        ...
        starkLines.data[position].shift(), starkLines.data[position].push({
    ...
    window.onload = function() {
        ...
        window.gapi.client.setApiKey(apiKey), window.setTimeout(checkAuth, 1);
    };
    ...
    var x = window.d3.scale.linear().domain([ 1, starkLines.data[0].length - 2 ]).range([ 0, window.innerWidth ]), y = window.d3.scale.linear().domain([ 0, window.d3.max(starkLines.data, function(d) {
        return d.value;
    }) ]).rangeRound([ 0, h ]), line = window.d3.svg.line().x(function(d, i) {
        return x(i);
    }).y(function(d) {
        return h - y(d.value) - .5;
    }).interpolate("basis"), w = window.innerWidth / starkLines.data[0].length, h = window.innerHeight / 2, color = function(int) {
    ...
    }, chart = (window.d3.select("#hero-img").append("svg").attr("class", "hero-chart").attr("width", window.innerWidth).attr("height", 350), 
    window.d3.select("#graph1").append("svg").attr("class", "chart").attr("width", w * starkLines.data[0].length - 1).attr("height", h)), runnable = (window.d3.select("#divvy"), 
    function() {
        for (starkLines.running = !0, i = 0; i < starkLines.data.length; i++) chart.append("path").attr("id", "myPath" + i.toString()).attr("stroke", color(i)).attr("stroke-width", 3).attr("fill", "none").attr("d", line(starkLines.data[i]));
        for (i = 0; i < starkLines.profileList.length; i++) setTimeout(queryLiveReportingApi, 110, starkLines.profileList[i].id);
        redraw(), starkLines.runningInterval = setInterval(function() {
            for (i = 0; i < starkLines.profileList.length; i++) setTimeout(queryLiveReportingApi, 110, starkLines.profileList[i].id);
            redraw(), window.d3.timer.flush();
        }, starkLines.delay);
    }), redraw = function() {
        for (y = window.d3.scale.linear().domain([ 0, 1 + parseInt(window.d3.max(starkLines.data, function(dat) {
            return window.d3.max(dat, function(d) {
                return parseInt(d.value, 10);
            });
        }), 10) ]).rangeRound([ 0, h ]), i = 0; i < starkLines.data.length; i++) chart.selectAll("#myPath" + i.toString()).attr("d", line(starkLines.data[i])).attr("transform", "translate(" + x(1) + ")").transition().ease("linear").duration(starkLines.delay - 50).attr("transform", "translate(" + x(0) + ")");
    };
    !function() {
